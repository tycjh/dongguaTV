...
个别源站会封代理出口 IP 或把播放地址签名绑定客户端 IP——这类线路**经代理必失败、直连却正常**。播放失败时客户端自动分诊：

- **代理活着（探测有任何 HTTP 响应）+ 还有别的线路** → 判定为"该线路过不了代理"：自动换下一条线路，**保住广告过滤**（该线路本会话记账，再选中它就直连播放并在控制台说明"过滤对该线路不可用"）；
- **代理不通/超时、或已有多条线路代理失败（疑似代理全局故障）、或没有备选线路** → 回退直连播放并弹提示"可能含广告"——能看优先，且不再是静默回退。
